...
library(rgdal)   # Replace with sf rgdal stopping 2023

#load input raster and shapefile
# note St_read best but takes time
in_raster<- raster(here('data','raw', 'raster', 'Pillar1','Air_pollution','ACAG_PM25_GWR_V4GL03_201801_201812_0p01-NoNegs-SIGDIG1_clip_all_countries.tif'))
...
#Creating a boolean map - Select pixels with air_pollution greater than 10 μg m3
in_ras_phase1_pilot_country_clip[in_ras_phase1_pilot_country_clip > 10] <- 1
plot(in_ras_phase1_pilot_country_clip)
plot(phase1_pilot_country, add=TRUE,col =NA, lwd=0.1,border="black")
...
csv_var <- c('pop_exposed_Air_pollution_grt_10_adm2_')

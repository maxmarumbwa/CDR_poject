...
inpath <- here('data','raw', 'raster')
in_raster<- raster(paste0(inpath, '/', 'Pillar1', '/','Cyclonic_wind_RT100years/','Cyclonic wind_RT100years.grd'))
pop_raster<- raster(paste0(inpath, '/', 'Population','/', 'gpw_v4_population_count_adjusted_to_2015_unwpp_country_totals_rev11_2020_30_sec.tif'))
...
#Creating a boolean map - Select pixels with wind speed Greater than 119km/hr
in_ras_phase1_pilot_country_clip[in_ras_phase1_pilot_country_clip > 119] <- 1
plot(in_ras_phase1_pilot_country_clip)
plot(phase1_pilot_country, add=TRUE,col =NA, lwd=0.1,border="black")
...
csv_var <- c('pop_exposed_Tropical_cyclone_119km_adm2_')
